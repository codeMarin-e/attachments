...
@php
    ...
    $edit_attachments = isset($attachable)?
        $attachable->attachments()->where('type', $type)->orderBy('ord', 'ASC')->get()->keyBy('id') : collect();
    if($oldOrder = old("{$inputBag}.{$type}")) {
        $attachments = collect();
...
<script>
    ...
    const inputElement = document.querySelector('.js_filepond');
